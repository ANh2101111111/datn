...
//                    List.of("Vui lòng cung cấp ít nhất một tiêu chí tìm kiếm."),
//                    List.of(),
//                    List.of(),
//                    List.of(),
//                    List.of()
...
public class ChatbotService {
    ...
    private final CategoryRepository categoryRepository;
    private final ProductMapper productMapper;
    private final GeminiAIService geminiAIService; // Thêm vào để gọi Gemini AI

    public ChatbotService(ProductRepository productRepository, CategoryRepository categoryRepository, ProductMapper productMapper, GeminiAIService geminiAIService) {
        this.productRepository = productRepository;
        this.categoryRepository = categoryRepository;
        this.productMapper = productMapper;
        this.geminiAIService = geminiAIService; // Khởi tạo Gemini AI
    }
    ...
    public ChatbotResponseDto getChatbotResponse(String message) {
        ...
        if (message.toLowerCase().contains("chào") || message.toLowerCase().contains("hello")) {
            return new ChatbotResponseDto(
                    List.of("Xin chào! Tôi có thể giúp gì cho bạn."),
                    List.of(),
                    List.of(),
                    List.of(),
                    List.of()
        ...
        if (message.toLowerCase().contains("có bao nhiêu sản phẩm")) {
            return new ChatbotResponseDto(
                    List.of("Hiện tại có 62 sản phẩm."),
                    List.of(),
                    List.of(),
                    List.of(),
                    List.of()
        ...
        if (message.toLowerCase().contains("có bao nhiêu danh mục")) {
            return new ChatbotResponseDto(
                    List.of("Hiện tại có 9 danh mục."),
                    List.of(),
                    List.of(),
                    List.of(),
                    List.of()
        ...
        // Nếu không tìm thấy sản phẩm và không có tiêu chí tìm kiếm
        if ( (name == null && type == null && categoryName == null && minPrice == null && maxPrice == null)) {
            return new ChatbotResponseDto(
                    List.of("Vui lòng cung cấp ít nhất một tiêu chí tìm kiếm."),
                    List.of(),
